...
parser.add_argument('--net',type=str,default='polar',help='polar or raw')
...
if list(net.parameters()):
    ...
    for m in list(net.parameters()):
        # fill input tensor with values picked from normal distribution
...
# graph hidden units
shutil.rmtree('spiral_images')
os.mkdir('spiral_images')
...
for layer in [1,2]:
    if layer == 1 or args.net != 'polar':
        for node in range(args.hid):
            ...
            plt.savefig('spiral_images/%s%d_%d.png' % (args.net, layer, node))

# graph output unit
graph_output(net)
plt.scatter(full_input[:,0],full_input[:,1],
            c=1-full_target[:,0],cmap='RdYlBu')
plt.savefig('spiral_images/%s_out.png' % args.net)
